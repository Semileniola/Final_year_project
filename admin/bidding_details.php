...
?> 
        ...
        <div class="col-md-8 text-end">
                    ...
                    <input type="hidden" name="_token" value="ykHaXp0IHU0FCpCrh5d2D9Vp99itEFbGWEjqtF9g" autocomplete="off">                    <button type="submit" class="eg-btn btn--green back-btn bids_close"> <img
                            src="https://www.bidout-app.egenslab.com/backend/images/icons/power.svg" alt="Bid Close"
                            width="15"> Bid Close</button>
                </form>-->
        ...
        <div class="col-lg-4">
            <div class="eg-card prod-details-card">
                ...
                <div class="prod-details-img p-3 mt-5">
                    <img  src="../uploads/products/features/<?php $getimages = $row['img_fname'];
// Use substr() and strpos() function to remove portion of string after comma
$featureimage = substr($getimages, 0, strpos($getimages, ",")); 

echo $featureimage; ?>" onerror="this.onerror=null;this.src='uploads/placeholder.jpg';" width="200"
                        alt="Product Image">
                </div>
                <div class="prod-content">
                    ...
                    <p>Product ID: 28</p>
                            ...
                            <h4>Bid Ending Time</h4>
                            <input type="hidden" id="bid_end_time"
                                value="<?php echo date('M j, Y H:i:s', strtotime($row['bid_end_datetime'])); ?>">
                        ...
                        <a href="#" class="eg-btn btn--primary prod-details-btn">Total Bids :
                            <span><?php echo countdata('product_id', $row['id'],'bids'); ?></span></a>
                    </div>
                            </div>
                                                </div>
        <div class="col-lg-8">
...
<?php
	...
	echo '  <tr>
                                    <td data-label="User"><a href="#"
                                            class="username">@ '.valueOfid2($bids['user_id'], 'username', 'users').'</a></td>
                                    <td data-label="Email / Phone"> <a
                                            href="mailto: '.valueOfid2($bids['user_id'], 'email', 'users').'"> '.valueOfid2($bids['user_id'], 'email', 'users').'</a> <br><a
                                            href="tel: '.valueOfid2($bids['user_id'], 'contact', 'users').'"
                                            class="phone"> '.valueOfid2($bids['user_id'], 'contact', 'users').'</a></td>
                                    <td data-label="Bidding Price">	&#8358;'.number_format($bids['bid_amount']).'</td>
                                    <td data-label="Bid Place Time">
                                        <p>'.date('M j, Y', strtotime($bids['date_created'])).'</p>
                                        <span class="time">'.date(' g:i A', strtotime($bids['date_created'])).'</span>
                                    </td>
                                    <td data-label="Status">'.$bidstat.'</td>

                                    <td data-label="Winner">
                                                                                    <form method="POST" action="../forms/action_handler.php">
                                                                                        
												<input name="bid" type="hidden" value="'.$bids['id'].'">
													<input name="getproduct" type="hidden" value="'.$getproduct.'">
														<input name="getprodname" type="hidden" value="'.$getprodname.'">
														<input name="crown_winner" type="hidden" value="0">
                                                                                <button type="submit" name="crown_winner"
                                            class=" winner_confirm  winner_btn"
