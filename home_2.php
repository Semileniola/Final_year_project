<?php include 'addon/head.php'; ?>
<body>

    
    <div class="layout-wrapper">
      

<?php include 'addon/topbar.php'; ?>

  
     

<?php include 'addon/header.php'; ?>

        <!-- ===============  Hero area end=============== -->

        <!-- main-container -->
        
        <div class="main-content">
            

    
                                    <div class="hero-area hero-style-one">
        <div class="hero-main-wrapper position-relative">
            <div class="swiper banner1">
                <div class="swiper-wrapper">
                                            <div class="swiper-slide">
                            <div class="slider-bg-1">
                                <style>
                                    .hero-style-one .slider-bg-1 {
                                        padding: 160px 0px;
                                        width: 100%;
                                        overflow: hidden;
                                        height: 100%;
                                        z-index: 1;
                                        position: relative;
                                    }

                                    @media (min-width: 1200px) and (max-width: 1399px) {
                                        .hero-style-one .slider-bg-1 {
                                            padding: 160px 0px;
                                        }
                                    }

                                    @media (min-width: 992px) and (max-width: 1199px) {
                                        .hero-style-one .slider-bg-1 {
                                            padding: 160px 0px;
                                        }
                                    }

                                    @media (max-width: 991px) {
                                        .hero-style-one .slider-bg-1 {
                                            padding: 120px 0px;
                                        }
                                    }

                                    .hero-style-one .slider-bg-1::before {
                                        content: "";

                                                                                    background-image: url(uploads/sliders/banner-bg11-1694236521.png) !important;
                                                                                background-size: cover;
                                        position: absolute;
                                        width: 100%;
                                        height: 100%;
                                        top: 0;
                                        left: 0;
                                        right: 0;
                                        z-index: -9;
                                        -webkit-animation: large 26s linear infinite alternate;
                                        animation: large 26s linear infinite alternate;
                                    }

                                    .hero-style-one .slider-bg-1::after {
                                        content: "";
                                        position: absolute;
                                        width: 100%;
                                        height: 100%;
                                        top: 0;
                                        left: 0;
                                        z-index: -9;
                                        background: rgba(0, 0, 0, 0.55);
                                    }

                                    @-webkit-keyframes large {
                                        0% {
                                            transform: scale(1);
                                        }

                                        100% {
                                            transform: scale(1.6);
                                        }
                                    }
                                </style>

                                <div class="container">
                                    <div class="row d-flex justify-content-center align-items-center">
                                        <div class="col-xl-10 col-lg-10">
                                            <div class="banner1-content">
                                                <span>Welcome To Bidout</span>
                                                <h1>Purchase Your Dream Product</h1>
                                                <p>Join us virtually for Meta Connect and tune in to sessions featuring Meta Quest 3, AI and Metaverse. You&#039;ll also get a first look at new products and updates to the Meta Presence Platform, Meta Spark, and more. Register now in Bidout.</p>
                                                                                                    <a href="all-products.php"
                                                        class="eg-btn btn--primary btn--lg">Start Exploring</a>
                                                                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                                            <div class="swiper-slide">
                            <div class="slider-bg-2">
                                <style>
                                    .hero-style-one .slider-bg-2 {
                                        padding: 160px 0px;
                                        width: 100%;
                                        overflow: hidden;
                                        height: 100%;
                                        z-index: 1;
                                        position: relative;
                                    }

                                    @media (min-width: 1200px) and (max-width: 1399px) {
                                        .hero-style-one .slider-bg-2 {
                                            padding: 160px 0px;
                                        }
                                    }

                                    @media (min-width: 992px) and (max-width: 1199px) {
                                        .hero-style-one .slider-bg-2 {
                                            padding: 160px 0px;
                                        }
                                    }

                                    @media (max-width: 991px) {
                                        .hero-style-one .slider-bg-2 {
                                            padding: 120px 0px;
                                        }
                                    }

                                    .hero-style-one .slider-bg-2::before {
                                        content: "";

                                                                                    background-image: url(uploads/sliders/banner-bg22-1694236546.png) !important;
                                                                                background-size: cover;
                                        position: absolute;
                                        width: 100%;
                                        height: 100%;
                                        top: 0;
                                        left: 0;
                                        right: 0;
                                        z-index: -9;
                                        -webkit-animation: large 26s linear infinite alternate;
                                        animation: large 26s linear infinite alternate;
                                    }

                                    .hero-style-one .slider-bg-2::after {
                                        content: "";
                                        position: absolute;
                                        width: 100%;
                                        height: 100%;
                                        top: 0;
                                        left: 0;
                                        z-index: -9;
                                        background: rgba(0, 0, 0, 0.55);
                                    }

                                    @-webkit-keyframes large {
                                        0% {
                                            transform: scale(1);
                                        }

                                        100% {
                                            transform: scale(1.6);
                                        }
                                    }
                                </style>

                                <div class="container">
                                    <div class="row d-flex justify-content-center align-items-center">
                                        <div class="col-xl-10 col-lg-10">
                                            <div class="banner1-content">
                                                <span>Welcome To Bidout</span>
                                                <h1>Create, sell and collect digital items</h1>
                                                <p>Join us virtually for Meta Connect and tune in to sessions featuring Meta Quest 3, AI and Metaverse. You&#039;ll also get a first look at new products and updates to the Meta Presence Platform, Meta Spark, and more. Register now at Bidout.</p>
                                                                                                    <a href="browse-products.php"
                                                        class="eg-btn btn--primary btn--lg">Start Exploring</a>
                                                                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                                    </div>
            </div>
            <div class="hero-one-pagination d-flex justify-content-center flex-column align-items-center gap-3"></div>
        </div>
    </div>
                                <div class="category-section pt-120">
        <div class="container position-relative">
		
		
            <div class="row d-flex justify-content-center">
			
			
			
                <div class="swiper category1-slider">
				
				
                    <div class="swiper-wrapper">
					
	
<?php
	$result3 = mysqli_query($dbc,"SELECT * FROM `categories` ORDER BY `id` DESC limit 7");
										if ($result3->num_rows > 0) 
										{
									
    while($row = mysqli_fetch_array($result3)){	
	//shortening notice text
	$catname = $row['name'];

	echo '<div class="swiper-slide">
                                <div class="eg-card category-card1 wow animate fadeInDown" data-wow-duration="1500ms"
                                    data-wow-delay="200ms">
                                    <div class="cat-icon">
                                        <img src="uploads/category/'.$row['icon'].'"
                                            alt="'.$catname.'" width="60" height="60">
                                    </div>
                                    <a href="browse_category.php?category='.$catname.'.php">
                                        <h5>'.$catname.'</h5>
                                    </a>
                                </div>
                            </div>';

	}
	
										}else {
		echo"<tr>
		<td colspan='4'>No category yet!</td>
		</tr>";									
										}
?>

							
                                            </div>
											
											
											
                </div>
				
				
				
            </div>
			
			
			
            <div class="slider-arrows text-center d-xl-flex d-none  justify-content-end">
                <div class="category-prev1 swiper-prev-arrow" tabindex="0" role="button" aria-label="Previous slide">
                    <i class='bx bx-chevron-left'></i>
                </div>
                <div class="category-next1 swiper-next-arrow" tabindex="0" role="button" aria-label="Next slide"> <i
                        class='bx bx-chevron-right'></i></div>
            </div>
        </div>
    </div>
                                <div class="live-auction pt-120">
    <input type="hidden" id="live_limit" value="6">
    <img alt="image" src="frontend/images/bg/section-bg.png" class="img-fluid section-bg">
    <div class="container position-relative">
        <img alt="image" src="frontend/images/bg/dotted1.png" class="dotted1">
        <img alt="image" src="frontend/images/bg/dotted1.png" class="dotted2">
        <div class="row d-flex justify-content-center">
            <div class="col-sm-12 col-md-10 col-lg-8 col-xl-6">
                <div class="section-title1">
                    <h2>Live Auction</h2>
                    <p class="mb-0">Discover unique treasures - bid and win! Uncover the exclusive world at your fingertips
Auction extravaganza</p>
                </div>
            </div>
        </div>
        <div class="row gy-4 d-flex justify-content-center">
		
		


	
<?php
	$result3 = mysqli_query($dbc,"SELECT * FROM `products` WHERE `bid_end_datetime` >  '$today' AND `bid_start_datetime` <= '$today' ORDER BY `date_created`");
										if ($result3->num_rows > 0) 
										{
									
    while($row = mysqli_fetch_array($result3)){	
	//shortening notice text
	$prodname = $row['name'];

	echo '
                                                <div class="col-lg-4 col-md-6 col-sm-10">
                        <div data-wow-duration="1.5s" data-wow-delay="0.2s"
    class="eg-card auction-card1 style-2 wow fadeInDown">
    <div class="auction-img">
                    <span class="product-type">Auction</span>
                
                    <img alt="Acution image" src="uploads/products/features/'.$row['img_fname'].'">
        
                                    <div class="auction-timer">
                <div class="countdown" id="timer'.$row['id'].'"
                    data-live-end-date="'.date('M j, Y H:i:s', strtotime($row['bid_end_datetime'])).'">
                    <h4><span id="days'.$row['id'].'"></span>D : <span id="hours'.$row['id'].'"></span>H :
                        <span id="minutes'.$row['id'].'"></span>M : <span id="seconds'.$row['id'].'"></span>S
                    </h4>
                </div>
            </div>
                <div class="author-area">
            <div class="author-emo">
                                    <img src="uploads/shop/f3404b1c3ee2878ebfd3d4c8b7acc510-1704801343.jpg" alt="Shop Logo">
                
            </div>
            <div class="author-name">
                <span><a href="shop/xyz-tread.php"> by
                        '.$row['userid'].'</a></span>
            </div>
        </div>
    </div>
    <div class="auction-content">
        <h4><a
                href="product/poutsicle-hydrating-lip-stain-2.php">'.$prodname.'</a>
        </h4>
        
            
                            <p>Current bid:
                    <span>N'.$row['regular_price'].'</span>
                </p>
                            <div class="auction-card-bttm">
                            <a href="product_details.php?product='.$row['id'].'&prodname='.$row['name'].'"
                    class="eg-btn btn--primary btn--sm">Place a Bid</a>
                        <div class="share-area">
                <ul class="social-icons d-flex">
                    <li><a target="_blank"
                            href="https://www.facebook.com/sharer.php?u='.$url.'/product/poutsicle-hydrating-lip-stain-2"><i
                                class="bx bxl-facebook"></i></a></li>
                    <li><a target="_blank"
                            href="https://twitter.com/intent/tweet?url='.$url.'/product/poutsicle-hydrating-lip-stain-2&amp;text=Poutsicle%20Hydrating%20Lip%20Stain."><i
                                class="bx bxl-twitter"></i></a></li>
                    <li><a target="_blank"
                            href="https://www.linkedin.com/shareArticle?url='.$url.'/product/poutsicle-hydrating-lip-stain-2&amp;title=Poutsicle%20Hydrating%20Lip%20Stain."><i
                                class="bx bxl-linkedin"></i></a></li>
                    <li><a target="_blank"
                            href="https://www.pinterest.com/pin/create/bookmarklet/?url=='.$url.'/product/poutsicle-hydrating-lip-stain-2"><i
                                class="bx bxl-pinterest"></i></a></li>
                </ul>
                <div>
                    <a href="#" class="share-btn"><i class="bx bxs-share-alt"></i></a>
                </div>
            </div>
        </div>
    </div>

</div>
                    </div>';

	}
	
										}else {
		echo"<tr>
		<td colspan='4'>No Live Auction!</td>
		</tr>";									
										}
?>


	
		
		
		
					
					
					
					
					







								   </div>
								   
								   
        <div class="row d-flex justify-content-center pt-60">
            <div class="col-md-4 text-center">
                <a target="__blank" href="auction-product.php"
                    class="eg-btn btn--primary btn--md mx-auto">View All</a>
            </div>
        </div>
    </div>
</div>


                                <div class="upcoming-seciton pt-120">
    <input type="hidden" id="upcoming_limit" value="3">
    <img alt="image" src="frontend/images/bg/section-bg.png" class="img-fluid section-bg">
    <div class="container">
        <div class="row d-flex justify-content-center">
            <div class="col-sm-12 col-md-10 col-lg-8 col-xl-6">
                <div class="section-title1">
                    <h2>upcomming Auction</h2>
                    <p class="mb-0">Your Gateway to Rare Finds Place your bid, seize the moment and own the extraordinary Enhance your collection with our exclusive auctions.</p>
                </div>
            </div>
        </div>
        <div class="row d-flex justify-content-center">
                            <div class="swiper upcoming-slider">
							
							
							
                    <div class="swiper-wrapper">
					
	
<?php
	$result3 = mysqli_query($dbc,"SELECT * FROM `products` WHERE `bid_start_datetime` > '$today' ORDER BY `date_created`");
										if ($result3->num_rows > 0) 
										{
									
    while($row = mysqli_fetch_array($result3)){	
	//shortening notice text
	$prodname = $row['name'];

	echo '<div class="swiper-slide">
                                <div class="eg-card c-feature-card1 wow animate fadeInDown" data-wow-duration="1.5s"
                                    data-wow-delay="0.2s">
                                    <div class="auction-img">

                                        <img alt="Acution image"
                                            src="uploads/products/features/'.$row['img_fname'].'">
                                                                                    <div class="auction-timer2 gap-lg-3 gap-md-2 gap-1"
                                                id="upcoming0"
                                                data-upcoming-start-date="'.date('M j, Y H:i:s', strtotime($row['bid_start_datetime'])).'">
                                                <div class="countdown-single">
                                                    <h5 id="upcoming_days0"></h5>
                                                    <span>Days</span>
                                                </div>
                                                <div class="countdown-single">
                                                    <h5 id="upcoming_hours0"></h5>
                                                    <span>Hours</span>
                                                </div>
                                                <div class="countdown-single">
                                                    <h5 id="upcoming_minutes0"></h5>
                                                    <span>Mins</span>
                                                </div>
                                                <div class="countdown-single">
                                                    <h5 id="upcoming_seconds0"></h5>
                                                    <span>Secs</span>
                                                </div>
                                            </div>
                                                                                <div class="author-area2">
                                            <a class="d-flex"
                                                href="shop/fair-shop.php">
                                                <div class="author-name">
                                                    <span>by
                                                        @Fair Shop</span>
                                                </div>
                                                <div class="author-emo">
                                                                                                            <img src="uploads/shop/360_F_413689616_TAnxJeZs5NOm4B4g9BAYXUFos5Lz6p9Q-1703596353.jpg"
                                                            alt="Shop Logo">
                                                    

                                                </div>
                                            </a>
                                        </div>
                                    </div>
                                    <div class="c-feature-content">
                                        <div class="c-feature-category">
                                            </div>
                                        <a href="product/luminance-dreams-digital-art-canvas-collection-2.php">
                                            <h4>'.$row['name'].'</h4>
                                        </a>
                                                                                    <p>Bidding Price :
                                                <span>N'.$row['regular_price'].'</span>
                                            </p>
                                        
                                        <div class="auction-card-bttm">
                                            <a href="product/luminance-dreams-digital-art-canvas-collection-2.php"
                                                class="eg-btn btn--primary btn--sm">View Details</a>
                                            <div class="share-area">
                                                <ul class="social-icons d-flex">
                                                    <li><a href="https://www.facebook.com/"><i
                                                                class="bx bxl-facebook"></i></a></li>
                                                    <li><a href="https://www.twitter.com/"><i
                                                                class="bx bxl-twitter"></i></a></li>
                                                    <li><a href="https://www.pinterest.com/"><i
                                                                class="bx bxl-pinterest"></i></a></li>
                                                    <li><a href="https://www.instagram.com/"><i
                                                                class="bx bxl-instagram"></i></a></li>
                                                </ul>
                                                <div>
                                                    <div class="share-btn"><i class="bx bxs-share-alt"></i></div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>';

	}
	
										}else {
		echo"<tr>
		<td colspan='4'>No Live Auction!</td>
		</tr>";									
										}
?>
				
					
					
                                                    
							
							
							
                                                    
                                                  
                                            </div>
                </div>
				
				
				
                <div class="slider-bottom d-flex justify-content-between align-items-center">
                    <a href="auction-product.php" class="eg-btn btn--primary btn--md">View All</a>
                    <div class="swiper-pagination style-3 d-lg-block d-none"></div>

                    <div class="slider-arrows coming-arrow d-flex gap-3">
                        <div class="coming-prev1 swiper-prev-arrow" tabindex="0" role="button"
                            aria-label="Previous slide"><i class="bi bi-arrow-left"></i></div>
                        <div class="coming-next1 swiper-next-arrow" tabindex="0" role="button"
                            aria-label="Next slide"><i class="bi bi-arrow-right"></i></div>
                    </div>
                </div>
				
				
				
                    </div>
    </div>
</div>
                                <div class="testimonial-section pt-80 pb-80 mt-120">
    <img alt="image" src="frontend/images/bg/client-right.png" class="client-right-vector">
    <img alt="image" src="frontend/images/bg/client-left.png" class="client-left-vector">
    <img alt="image" src="frontend/images/bg/clent-circle1.png" class="client-circle1">
    <img alt="image" src="frontend/images/bg/clent-circle2.png" class="client-circle2">
    <img alt="image" src="frontend/images/bg/clent-circle3.png" class="client-circle3">
    <div class="container">
        <div class="row d-flex justify-content-center">
            <div class="col-sm-12 col-md-10 col-lg-8 col-xl-6">
                <div class="section-title1">
                    <h2>What Client Say</h2>
                    <p class="mb-0">I&#039;ve been using it to sell my items, and I couldn&#039;t be happier with the results. The platform is user-friendly, and I was able to reach a wide audience for my auctions. The seamless bidding process and secure transactions make it my go-to choice for online auctions.</p>
                </div>
            </div>
        </div>
                    <div class="row justify-content-center position-relative">
                <div class="swiper testimonial-slider">
                    <div class="swiper-wrapper">
                                                    <div class="swiper-slide">
                                <div class="testimonial-single hover-border1 wow fadeInDown" data-wow-duration="1.5s"
                                    data-wow-delay=".2s">


                                    

                                    <div class="quote-icon">
                                        <svg width="38" height="28" viewBox="0 0 38 28"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                                                d="M6.70166 1.32327C3.44346 4.49241 1.15752 8.87874 0.296586 13.5916C-0.0077112 15.2912 -0.0967737 18.3194 0.118461 19.7666C0.341117 21.2733 0.905179 22.7354 1.71416 23.908C3.72549 26.8248 7.41416 28.1014 10.895 27.092C12.2606 26.6987 13.3665 26.0307 14.4427 24.9545C15.4892 23.908 16.0458 23.0174 16.4837 21.7112C17.2036 19.5662 17.0478 17.2358 16.0606 15.2393C14.8657 12.8123 12.4759 11.0385 9.90791 10.66C8.83916 10.5041 8.95049 10.6006 9.00245 9.85101C9.11377 8.29241 9.7001 6.71155 10.8134 4.92288C11.4665 3.89124 11.5259 4.03968 9.64073 2.15452C7.65166 0.172878 7.83721 0.224833 6.70166 1.32327ZM9.50713 4.59632C8.77979 5.79124 8.16377 7.3053 7.90401 8.58929C7.74073 9.36858 7.71104 10.8752 7.85205 11.276C7.99307 11.6545 8.1712 11.751 8.90596 11.803C11.6001 12.0182 13.9083 13.6065 15.029 16.026C15.5263 17.0948 15.6228 17.5994 15.6153 19.0096C15.6153 20.1674 15.5931 20.3307 15.4075 20.9393C14.6579 23.4034 12.706 25.2959 10.2419 25.9565C9.3587 26.194 7.58487 26.2014 6.73877 25.9639C3.80713 25.1549 1.80323 22.728 1.33565 19.4252C1.22432 18.6088 1.23916 16.0409 1.37276 15.0018C1.57315 13.3764 1.90713 11.944 2.41182 10.5116C3.45088 7.56507 5.21729 4.6928 7.41416 2.39944L7.75557 2.05061L8.77979 3.07483L9.81143 4.10647L9.50713 4.59632Z" />
                                            <path
                                                d="M27.4979 1.62755C23.9131 5.27912 21.6791 10.133 21.1447 15.4322C21.0186 16.7088 21.1002 19.3955 21.2858 20.3084C22.1541 24.4721 25.2268 27.2405 29.1752 27.4112C31.5576 27.5151 33.658 26.7432 35.3576 25.1326C36.7084 23.8487 37.5545 22.3049 37.8885 20.494C38.0592 19.5959 38.0072 17.7776 37.792 16.9612C36.9459 13.7846 34.5857 11.469 31.4315 10.7342C31.0826 10.6526 30.6744 10.5858 30.5186 10.5858C30.0584 10.5858 30.0287 10.5338 30.0881 9.80646C30.2143 8.28498 30.7709 6.77834 31.8916 4.93029C32.2924 4.27716 32.3666 4.10646 32.3295 3.92834C32.2924 3.7799 31.8174 3.24552 30.7783 2.20646C29.9471 1.38263 29.2197 0.670134 29.1604 0.633024C29.101 0.595915 28.9377 0.566227 28.7967 0.566227C28.5592 0.566227 28.4479 0.66271 27.4979 1.62755ZM30.3924 4.94513C29.2791 6.89709 28.6705 9.19045 28.8635 10.7268C28.9822 11.6842 29.0342 11.7287 29.9916 11.8029C33.0865 12.0479 35.5729 14.0444 36.4932 17.0205C36.6787 17.6217 36.6936 17.7701 36.6936 18.9725C36.6936 20.1748 36.6787 20.3233 36.4932 20.9244C35.8326 23.0545 34.5264 24.5834 32.5596 25.5037C31.5131 25.9936 30.86 26.1346 29.576 26.1346C28.6483 26.1346 28.3588 26.1049 27.8318 25.9639C25.2861 25.2811 23.3193 23.2104 22.644 20.5162C22.4065 19.5811 22.3471 18.9205 22.3397 17.4139C22.3397 14.1557 22.9928 11.2686 24.4104 8.32209C25.4494 6.16232 26.5479 4.5592 28.3217 2.61466L28.8338 2.05802L29.858 3.07482L30.8748 4.09162L30.3924 4.94513Z" />
                                        </svg>
                                    </div>





                                    <div class="testi-img">
                                        <img alt="image"
                                            src="uploads/testimonials/yannie_tan-1694255143.jpg">
                                    </div>
                                    <div class="testi-content">
                                        <p class="para">As a frequent buyer on I appreciate the diverse range of items available and the excitement of the bidding process. The website&#039;s intuitive design and transparent information about each listing make it easy to find and bid on items I love.</p>
                                        <div class="testi-designation">
                                            <h5><a href="#">Johan Martin</a></h5>
                                            <p>CEO</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                                    <div class="swiper-slide">
                                <div class="testimonial-single hover-border1 wow fadeInDown" data-wow-duration="1.5s"
                                    data-wow-delay=".2s">


                                    

                                    <div class="quote-icon">
                                        <svg width="38" height="28" viewBox="0 0 38 28"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                                                d="M6.70166 1.32327C3.44346 4.49241 1.15752 8.87874 0.296586 13.5916C-0.0077112 15.2912 -0.0967737 18.3194 0.118461 19.7666C0.341117 21.2733 0.905179 22.7354 1.71416 23.908C3.72549 26.8248 7.41416 28.1014 10.895 27.092C12.2606 26.6987 13.3665 26.0307 14.4427 24.9545C15.4892 23.908 16.0458 23.0174 16.4837 21.7112C17.2036 19.5662 17.0478 17.2358 16.0606 15.2393C14.8657 12.8123 12.4759 11.0385 9.90791 10.66C8.83916 10.5041 8.95049 10.6006 9.00245 9.85101C9.11377 8.29241 9.7001 6.71155 10.8134 4.92288C11.4665 3.89124 11.5259 4.03968 9.64073 2.15452C7.65166 0.172878 7.83721 0.224833 6.70166 1.32327ZM9.50713 4.59632C8.77979 5.79124 8.16377 7.3053 7.90401 8.58929C7.74073 9.36858 7.71104 10.8752 7.85205 11.276C7.99307 11.6545 8.1712 11.751 8.90596 11.803C11.6001 12.0182 13.9083 13.6065 15.029 16.026C15.5263 17.0948 15.6228 17.5994 15.6153 19.0096C15.6153 20.1674 15.5931 20.3307 15.4075 20.9393C14.6579 23.4034 12.706 25.2959 10.2419 25.9565C9.3587 26.194 7.58487 26.2014 6.73877 25.9639C3.80713 25.1549 1.80323 22.728 1.33565 19.4252C1.22432 18.6088 1.23916 16.0409 1.37276 15.0018C1.57315 13.3764 1.90713 11.944 2.41182 10.5116C3.45088 7.56507 5.21729 4.6928 7.41416 2.39944L7.75557 2.05061L8.77979 3.07483L9.81143 4.10647L9.50713 4.59632Z" />
                                            <path
                                                d="M27.4979 1.62755C23.9131 5.27912 21.6791 10.133 21.1447 15.4322C21.0186 16.7088 21.1002 19.3955 21.2858 20.3084C22.1541 24.4721 25.2268 27.2405 29.1752 27.4112C31.5576 27.5151 33.658 26.7432 35.3576 25.1326C36.7084 23.8487 37.5545 22.3049 37.8885 20.494C38.0592 19.5959 38.0072 17.7776 37.792 16.9612C36.9459 13.7846 34.5857 11.469 31.4315 10.7342C31.0826 10.6526 30.6744 10.5858 30.5186 10.5858C30.0584 10.5858 30.0287 10.5338 30.0881 9.80646C30.2143 8.28498 30.7709 6.77834 31.8916 4.93029C32.2924 4.27716 32.3666 4.10646 32.3295 3.92834C32.2924 3.7799 31.8174 3.24552 30.7783 2.20646C29.9471 1.38263 29.2197 0.670134 29.1604 0.633024C29.101 0.595915 28.9377 0.566227 28.7967 0.566227C28.5592 0.566227 28.4479 0.66271 27.4979 1.62755ZM30.3924 4.94513C29.2791 6.89709 28.6705 9.19045 28.8635 10.7268C28.9822 11.6842 29.0342 11.7287 29.9916 11.8029C33.0865 12.0479 35.5729 14.0444 36.4932 17.0205C36.6787 17.6217 36.6936 17.7701 36.6936 18.9725C36.6936 20.1748 36.6787 20.3233 36.4932 20.9244C35.8326 23.0545 34.5264 24.5834 32.5596 25.5037C31.5131 25.9936 30.86 26.1346 29.576 26.1346C28.6483 26.1346 28.3588 26.1049 27.8318 25.9639C25.2861 25.2811 23.3193 23.2104 22.644 20.5162C22.4065 19.5811 22.3471 18.9205 22.3397 17.4139C22.3397 14.1557 22.9928 11.2686 24.4104 8.32209C25.4494 6.16232 26.5479 4.5592 28.3217 2.61466L28.8338 2.05802L29.858 3.07482L30.8748 4.09162L30.3924 4.94513Z" />
                                        </svg>
                                    </div>





                                    <div class="testi-img">
                                        <img alt="image"
                                            src="uploads/testimonials/nadia_k-1694255143.jpg">
                                    </div>
                                    <div class="testi-content">
                                        <p class="para">Our company has been using it to liquidate excess inventory, and the results have exceeded our expectations. The platform&#039;s reach, combined with its efficient auction management tools, has streamlined our selling process and maximized returns.</p>
                                        <div class="testi-designation">
                                            <h5><a href="#">Jamie anderson</a></h5>
                                            <p>CTO</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                                    <div class="swiper-slide">
                                <div class="testimonial-single hover-border1 wow fadeInDown" data-wow-duration="1.5s"
                                    data-wow-delay=".2s">


                                    

                                    <div class="quote-icon">
                                        <svg width="38" height="28" viewBox="0 0 38 28"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                                                d="M6.70166 1.32327C3.44346 4.49241 1.15752 8.87874 0.296586 13.5916C-0.0077112 15.2912 -0.0967737 18.3194 0.118461 19.7666C0.341117 21.2733 0.905179 22.7354 1.71416 23.908C3.72549 26.8248 7.41416 28.1014 10.895 27.092C12.2606 26.6987 13.3665 26.0307 14.4427 24.9545C15.4892 23.908 16.0458 23.0174 16.4837 21.7112C17.2036 19.5662 17.0478 17.2358 16.0606 15.2393C14.8657 12.8123 12.4759 11.0385 9.90791 10.66C8.83916 10.5041 8.95049 10.6006 9.00245 9.85101C9.11377 8.29241 9.7001 6.71155 10.8134 4.92288C11.4665 3.89124 11.5259 4.03968 9.64073 2.15452C7.65166 0.172878 7.83721 0.224833 6.70166 1.32327ZM9.50713 4.59632C8.77979 5.79124 8.16377 7.3053 7.90401 8.58929C7.74073 9.36858 7.71104 10.8752 7.85205 11.276C7.99307 11.6545 8.1712 11.751 8.90596 11.803C11.6001 12.0182 13.9083 13.6065 15.029 16.026C15.5263 17.0948 15.6228 17.5994 15.6153 19.0096C15.6153 20.1674 15.5931 20.3307 15.4075 20.9393C14.6579 23.4034 12.706 25.2959 10.2419 25.9565C9.3587 26.194 7.58487 26.2014 6.73877 25.9639C3.80713 25.1549 1.80323 22.728 1.33565 19.4252C1.22432 18.6088 1.23916 16.0409 1.37276 15.0018C1.57315 13.3764 1.90713 11.944 2.41182 10.5116C3.45088 7.56507 5.21729 4.6928 7.41416 2.39944L7.75557 2.05061L8.77979 3.07483L9.81143 4.10647L9.50713 4.59632Z" />
                                            <path
                                                d="M27.4979 1.62755C23.9131 5.27912 21.6791 10.133 21.1447 15.4322C21.0186 16.7088 21.1002 19.3955 21.2858 20.3084C22.1541 24.4721 25.2268 27.2405 29.1752 27.4112C31.5576 27.5151 33.658 26.7432 35.3576 25.1326C36.7084 23.8487 37.5545 22.3049 37.8885 20.494C38.0592 19.5959 38.0072 17.7776 37.792 16.9612C36.9459 13.7846 34.5857 11.469 31.4315 10.7342C31.0826 10.6526 30.6744 10.5858 30.5186 10.5858C30.0584 10.5858 30.0287 10.5338 30.0881 9.80646C30.2143 8.28498 30.7709 6.77834 31.8916 4.93029C32.2924 4.27716 32.3666 4.10646 32.3295 3.92834C32.2924 3.7799 31.8174 3.24552 30.7783 2.20646C29.9471 1.38263 29.2197 0.670134 29.1604 0.633024C29.101 0.595915 28.9377 0.566227 28.7967 0.566227C28.5592 0.566227 28.4479 0.66271 27.4979 1.62755ZM30.3924 4.94513C29.2791 6.89709 28.6705 9.19045 28.8635 10.7268C28.9822 11.6842 29.0342 11.7287 29.9916 11.8029C33.0865 12.0479 35.5729 14.0444 36.4932 17.0205C36.6787 17.6217 36.6936 17.7701 36.6936 18.9725C36.6936 20.1748 36.6787 20.3233 36.4932 20.9244C35.8326 23.0545 34.5264 24.5834 32.5596 25.5037C31.5131 25.9936 30.86 26.1346 29.576 26.1346C28.6483 26.1346 28.3588 26.1049 27.8318 25.9639C25.2861 25.2811 23.3193 23.2104 22.644 20.5162C22.4065 19.5811 22.3471 18.9205 22.3397 17.4139C22.3397 14.1557 22.9928 11.2686 24.4104 8.32209C25.4494 6.16232 26.5479 4.5592 28.3217 2.61466L28.8338 2.05802L29.858 3.07482L30.8748 4.09162L30.3924 4.94513Z" />
                                        </svg>
                                    </div>





                                    <div class="testi-img">
                                        <img alt="image"
                                            src="uploads/testimonials/jeremy_mcKeehen-1694257879.jpg">
                                    </div>
                                    <div class="testi-content">
                                        <p class="para">Being a collector, I&#039;ve explored various auction platforms, and BidOut stands out for its diverse selection and reputable sellers. The platform provides a reliable and secure environment for both buyers and sellers.</p>
                                        <div class="testi-designation">
                                            <h5><a href="#">Jhon Peter</a></h5>
                                            <p>Manager</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                                    <div class="swiper-slide">
                                <div class="testimonial-single hover-border1 wow fadeInDown" data-wow-duration="1.5s"
                                    data-wow-delay=".2s">


                                    

                                    <div class="quote-icon">
                                        <svg width="38" height="28" viewBox="0 0 38 28"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                                                d="M6.70166 1.32327C3.44346 4.49241 1.15752 8.87874 0.296586 13.5916C-0.0077112 15.2912 -0.0967737 18.3194 0.118461 19.7666C0.341117 21.2733 0.905179 22.7354 1.71416 23.908C3.72549 26.8248 7.41416 28.1014 10.895 27.092C12.2606 26.6987 13.3665 26.0307 14.4427 24.9545C15.4892 23.908 16.0458 23.0174 16.4837 21.7112C17.2036 19.5662 17.0478 17.2358 16.0606 15.2393C14.8657 12.8123 12.4759 11.0385 9.90791 10.66C8.83916 10.5041 8.95049 10.6006 9.00245 9.85101C9.11377 8.29241 9.7001 6.71155 10.8134 4.92288C11.4665 3.89124 11.5259 4.03968 9.64073 2.15452C7.65166 0.172878 7.83721 0.224833 6.70166 1.32327ZM9.50713 4.59632C8.77979 5.79124 8.16377 7.3053 7.90401 8.58929C7.74073 9.36858 7.71104 10.8752 7.85205 11.276C7.99307 11.6545 8.1712 11.751 8.90596 11.803C11.6001 12.0182 13.9083 13.6065 15.029 16.026C15.5263 17.0948 15.6228 17.5994 15.6153 19.0096C15.6153 20.1674 15.5931 20.3307 15.4075 20.9393C14.6579 23.4034 12.706 25.2959 10.2419 25.9565C9.3587 26.194 7.58487 26.2014 6.73877 25.9639C3.80713 25.1549 1.80323 22.728 1.33565 19.4252C1.22432 18.6088 1.23916 16.0409 1.37276 15.0018C1.57315 13.3764 1.90713 11.944 2.41182 10.5116C3.45088 7.56507 5.21729 4.6928 7.41416 2.39944L7.75557 2.05061L8.77979 3.07483L9.81143 4.10647L9.50713 4.59632Z" />
                                            <path
                                                d="M27.4979 1.62755C23.9131 5.27912 21.6791 10.133 21.1447 15.4322C21.0186 16.7088 21.1002 19.3955 21.2858 20.3084C22.1541 24.4721 25.2268 27.2405 29.1752 27.4112C31.5576 27.5151 33.658 26.7432 35.3576 25.1326C36.7084 23.8487 37.5545 22.3049 37.8885 20.494C38.0592 19.5959 38.0072 17.7776 37.792 16.9612C36.9459 13.7846 34.5857 11.469 31.4315 10.7342C31.0826 10.6526 30.6744 10.5858 30.5186 10.5858C30.0584 10.5858 30.0287 10.5338 30.0881 9.80646C30.2143 8.28498 30.7709 6.77834 31.8916 4.93029C32.2924 4.27716 32.3666 4.10646 32.3295 3.92834C32.2924 3.7799 31.8174 3.24552 30.7783 2.20646C29.9471 1.38263 29.2197 0.670134 29.1604 0.633024C29.101 0.595915 28.9377 0.566227 28.7967 0.566227C28.5592 0.566227 28.4479 0.66271 27.4979 1.62755ZM30.3924 4.94513C29.2791 6.89709 28.6705 9.19045 28.8635 10.7268C28.9822 11.6842 29.0342 11.7287 29.9916 11.8029C33.0865 12.0479 35.5729 14.0444 36.4932 17.0205C36.6787 17.6217 36.6936 17.7701 36.6936 18.9725C36.6936 20.1748 36.6787 20.3233 36.4932 20.9244C35.8326 23.0545 34.5264 24.5834 32.5596 25.5037C31.5131 25.9936 30.86 26.1346 29.576 26.1346C28.6483 26.1346 28.3588 26.1049 27.8318 25.9639C25.2861 25.2811 23.3193 23.2104 22.644 20.5162C22.4065 19.5811 22.3471 18.9205 22.3397 17.4139C22.3397 14.1557 22.9928 11.2686 24.4104 8.32209C25.4494 6.16232 26.5479 4.5592 28.3217 2.61466L28.8338 2.05802L29.858 3.07482L30.8748 4.09162L30.3924 4.94513Z" />
                                        </svg>
                                    </div>





                                    <div class="testi-img">
                                        <img alt="image"
                                            src="uploads/testimonials/mahesh_fonseka-1694257879.jpg">
                                    </div>
                                    <div class="testi-content">
                                        <p class="para">Our charity organization has successfully hosted online fundraisers using Bidout. The platform&#039;s support for charitable auctions, coupled with its user-friendly interface, has allowed us to connect with generous donors and raise funds for our cause.</p>
                                        <div class="testi-designation">
                                            <h5><a href="#">Johan Martin</a></h5>
                                            <p>Asst. Manager</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                            </div>
                </div>
                <div class="slider-arrows testimonial2-arrow d-flex justify-content-between gap-3">
                    <div class="testi-prev1 swiper-prev-arrow" tabindex="0" role="button"
                        aria-label="Previous slide"><i class="bi bi-arrow-left"></i></div>
                    <div class="testi-next1 swiper-next-arrow" tabindex="0" role="button" aria-label="Next slide">
                        <i class="bi bi-arrow-right"></i>
                    </div>
                </div>
            </div>
            </div>
</div>
                                <div class="recent-news-section pt-120">
    <img alt="image" src="frontend/images/bg/section-bg.png" class="img-fluid section-bg">
    <div class="dot-circle">

        <svg width="93" height="92" viewBox="0 0 93 92" xmlns="http://www.w3.org/2000/svg">
            <g opacity="0.6">
                <path
                    d="M84.6083 66.1492C83.1094 65.5081 82.4084 63.7663 83.0494 62.2658C83.6967 60.7592 85.4324 60.0658 86.9405 60.7131C88.4395 61.3542 89.1405 63.096 88.4994 64.5949C88.4441 64.7118 88.3934 64.8179 88.3334 64.9209C87.5939 66.1923 85.9951 66.7473 84.6083 66.1492Z" />
                <path
                    d="M75.1506 82.3305C73.8792 83.3621 72.0159 83.1699 70.9844 81.9001C69.9482 80.6379 70.1403 78.7746 71.4025 77.7384C72.6739 76.7068 74.5372 76.899 75.5734 78.1612C76.3698 79.1482 76.4374 80.4888 75.8378 81.5203C75.6657 81.8186 75.4351 82.0953 75.1506 82.3305Z" />
                <path
                    d="M89.3697 45.8852C87.7432 46.0144 86.3149 44.7983 86.1781 43.1656C86.049 41.5391 87.265 40.1108 88.8916 39.9817C90.5227 39.8449 91.9509 41.0609 92.0878 42.6921C92.1324 43.3224 91.9848 43.9112 91.6927 44.4139C91.2238 45.2272 90.3751 45.8068 89.3697 45.8852Z" />
                <path
                    d="M55.0908 91.8112C53.4827 92.1448 51.9207 91.1009 51.5902 89.5067C51.2643 87.9032 52.3005 86.335 53.904 86.0107C55.5074 85.6863 57.074 86.7209 57.3999 88.3244C57.5537 89.0639 57.4153 89.7926 57.0633 90.3999C56.6512 91.1071 55.9548 91.639 55.0908 91.8112Z" />
                <path
                    d="M84.1717 25.7196C82.7773 26.5883 80.9555 26.1701 80.0823 24.7849C79.2167 23.4044 79.6272 21.5764 81.0124 20.7032C82.3975 19.83 84.2285 20.2527 85.0941 21.6333C85.706 22.6049 85.686 23.7795 85.151 24.7019C84.9173 25.1032 84.5868 25.4583 84.1717 25.7196Z" />
                <path
                    d="M32.911 90.9228C31.3444 90.4585 30.4373 88.8135 30.9016 87.2469C31.3521 85.6834 32.9971 84.7764 34.5683 85.233C36.1318 85.685 37.0342 87.3361 36.5822 88.9011C36.5146 89.1348 36.4162 89.3639 36.2978 89.5684C35.6352 90.7122 34.2408 91.3056 32.911 90.9228Z" />
                <path
                    d="M66.1476 11.3589C66.1215 11.3435 66.0969 11.3297 66.08 11.3189C66.0538 11.3036 66.0292 11.2897 66.0123 11.279C64.5902 10.4657 64.1014 8.65314 64.9238 7.23568C65.7371 5.81361 67.5451 5.33395 68.9595 6.14261C68.9933 6.16259 69.0194 6.17797 69.044 6.1918C69.0701 6.20718 69.0947 6.22101 69.1209 6.23639C70.5383 7.05888 71.018 8.86684 70.1955 10.2828C69.3715 11.7018 67.5635 12.1814 66.1476 11.3589Z" />
                <path
                    d="M17.8863 79.7293C16.7563 80.91 14.8823 80.9531 13.7016 79.8231C12.5209 78.6931 12.4824 76.8114 13.6139 75.6291C14.7439 74.4484 16.6211 74.4192 17.8018 75.5492C18.8072 76.51 18.984 78.0151 18.3152 79.1666C18.1953 79.3726 18.0554 79.554 17.8863 79.7293Z" />
                <path
                    d="M47.407 5.91827C45.7789 5.85217 44.5121 4.47775 44.569 2.84506C44.6259 1.21237 46.0064 -0.0636501 47.633 0.00245702C49.2733 0.0654894 50.5417 1.4399 50.4848 3.07259C50.4664 3.57531 50.3218 4.0396 50.0897 4.44086C49.5562 5.36174 48.5462 5.96286 47.407 5.91827Z" />
                <path
                    d="M5.5889 62.9288C4.03615 63.4423 2.36348 62.6091 1.85 61.0548C1.32268 59.5051 2.16978 57.8294 3.71484 57.3097C5.26758 56.7962 6.94025 57.6295 7.46757 59.1792C7.74276 60.0232 7.62592 60.8918 7.21083 61.6098C6.86338 62.2078 6.29763 62.6905 5.5889 62.9288Z" />
                <path
                    d="M28.442 9.8156C26.9692 10.5243 25.1996 9.89708 24.4955 8.41505C23.7991 6.93917 24.4279 5.16965 25.9038 4.47476C27.3842 3.77064 29.1461 4.39328 29.8502 5.87377C30.2883 6.79005 30.2069 7.81548 29.7318 8.6349C29.4443 9.12993 29.0062 9.54963 28.442 9.8156Z" />
                <path
                    d="M2.48648 42.3373C0.870698 42.0729 -0.223912 40.5586 0.0389786 38.9428C0.303407 37.327 1.81772 36.2324 3.43811 36.4876C5.05389 36.752 6.14389 38.274 5.88561 39.8821C5.82719 40.2588 5.69959 40.5955 5.52126 40.9029C4.92629 41.9268 3.73944 42.5387 2.48648 42.3373Z" />
                <path
                    d="M13.4664 22.076C12.4917 23.3813 10.6361 23.6503 9.32165 22.671C8.01642 21.6963 7.75199 19.8315 8.72515 18.5262C9.70446 17.2118 11.5555 16.952 12.8699 17.9313C14.086 18.8429 14.4042 20.4987 13.6601 21.7793C13.6017 21.8808 13.5341 21.9776 13.4664 22.076Z" />
                <path
                    d="M77.5565 62.549C76.3282 62.0309 75.7547 60.6011 76.2867 59.3697C76.814 58.1459 78.2422 57.5709 79.466 58.0983C80.6974 58.6302 81.2631 60.0538 80.7358 61.2775C80.7035 61.3729 80.6543 61.459 80.6036 61.5435C80.001 62.5859 78.6988 63.0394 77.5565 62.549Z" />
                <path
                    d="M69.8172 75.7887C68.7779 76.6358 67.2483 76.4775 66.4165 75.4351C65.5648 74.4051 65.7216 72.8754 66.7609 72.0298C67.8002 71.1827 69.3299 71.3411 70.1662 72.3742C70.8226 73.189 70.8749 74.2805 70.3845 75.1246C70.2415 75.3736 70.0555 75.5965 69.8172 75.7887Z" />
                <path
                    d="M81.4583 45.9728C80.1208 46.0758 78.9524 45.0781 78.8447 43.7498C78.7417 42.4123 79.7349 41.2531 81.0678 41.1378C82.3961 41.0302 83.5722 42.0326 83.6798 43.3608C83.7151 43.8712 83.5906 44.3586 83.3539 44.7691C82.968 45.4348 82.2762 45.9006 81.4583 45.9728Z" />
                <path
                    d="M53.4012 83.5513C52.0929 83.8188 50.8154 82.9748 50.5509 81.6573C50.2834 80.349 51.1275 79.0714 52.4358 78.8024C53.7441 78.5349 55.0216 79.3789 55.2984 80.6918C55.4152 81.296 55.3076 81.8956 55.0201 82.3906C54.6849 82.9718 54.1146 83.4053 53.4012 83.5513Z" />
                <path
                    d="M77.2044 29.4708C76.0636 30.178 74.5678 29.8459 73.8606 28.7052C73.1488 27.5737 73.4885 26.0824 74.6216 25.3691C75.7531 24.6573 77.2536 25.0016 77.9577 26.1285C78.4573 26.9203 78.445 27.8842 78.01 28.6344C77.8147 28.968 77.5441 29.2571 77.2044 29.4708Z" />
                <path
                    d="M35.2634 82.8159C33.9736 82.4439 33.2387 81.094 33.6138 79.8165C33.9859 78.5266 35.328 77.7872 36.6178 78.1592C37.8939 78.5343 38.6426 79.8811 38.2613 81.1678C38.2044 81.3631 38.1275 81.5353 38.0291 81.7059C37.4849 82.6453 36.358 83.1326 35.2634 82.8159Z" />
                <path
                    d="M62.4516 17.7268L62.4562 17.7191C62.447 17.7145 62.4393 17.7083 62.4224 17.6991C62.3963 17.6837 62.3655 17.6776 62.3409 17.6622C61.1802 16.9889 60.7805 15.513 61.4462 14.3476C62.1196 13.1869 63.5954 12.7872 64.7561 13.4606C64.7823 13.4759 64.7992 13.4852 64.8238 13.5005C64.8407 13.5098 64.8668 13.5251 64.8838 13.5359C66.0445 14.2093 66.4442 15.6852 65.7708 16.8459C65.0959 18.0066 63.6123 18.4017 62.4516 17.7268Z" />
                <path
                    d="M22.9689 73.6628C22.0419 74.6314 20.506 74.6636 19.5421 73.7289C19.5421 73.7289 19.5421 73.7289 19.5375 73.7366C18.5828 72.8065 18.5459 71.2783 19.4714 70.3098C20.3984 69.3412 21.9296 69.3166 22.9028 70.236C23.7191 71.0293 23.8652 72.2546 23.3148 73.2016C23.221 73.3661 23.1042 73.526 22.9689 73.6628Z" />
                <path
                    d="M47.1227 13.2716C45.7867 13.2147 44.7459 12.0986 44.7936 10.7565C44.8489 9.4205 45.9666 8.3797 47.3087 8.42735C48.6493 8.47501 49.6947 9.60652 49.6378 10.9425C49.6225 11.3438 49.5087 11.7342 49.3165 12.0679C48.8845 12.8119 48.0543 13.2993 47.1227 13.2716Z" />
                <path
                    d="M12.9036 59.9172C11.6383 60.3477 10.2623 59.6513 9.84572 58.3829C9.41525 57.1177 10.0978 55.7448 11.3708 55.3205C12.6453 54.8962 14.012 55.5864 14.4425 56.8517C14.6685 57.542 14.5716 58.2615 14.2349 58.8426C13.9536 59.3284 13.4878 59.7189 12.9036 59.9172Z" />
                <path
                    d="M31.6104 16.4692C30.3989 17.0442 28.9538 16.5245 28.3788 15.3131C27.8039 14.1016 28.3173 12.6657 29.5303 12.0908C30.7341 11.5112 32.1792 12.0293 32.7588 13.233C33.1093 13.9848 33.0525 14.8304 32.6666 15.496C32.4267 15.905 32.0701 16.2447 31.6104 16.4692Z" />
                <path
                    d="M10.37 43.0661C9.0525 42.8601 8.15621 41.6102 8.36222 40.2927C8.58207 38.9706 9.82273 38.0712 11.1449 38.2895C12.4639 38.4955 13.3587 39.7454 13.1527 41.0629C13.1035 41.3658 12.9974 41.6456 12.8529 41.8931C12.3625 42.7387 11.3893 43.2352 10.37 43.0661Z" />
                <path
                    d="M19.3515 26.4838C18.5551 27.5615 17.0377 27.7767 15.96 26.9804C14.887 26.1748 14.6717 24.6589 15.4773 23.5843C16.2767 22.5189 17.7987 22.296 18.8641 23.0954C19.8573 23.8426 20.1171 25.2032 19.5083 26.2532C19.4637 26.3316 19.4145 26.4161 19.3515 26.4838Z" />
                <path
                    d="M71.5347 59.4755C70.54 59.0466 70.0757 57.8982 70.5046 56.9035C70.9336 55.9088 72.0851 55.4368 73.089 55.8704C74.076 56.2947 74.5418 57.4554 74.1144 58.4501C74.0836 58.5239 74.0437 58.5915 74.0037 58.6592C73.5148 59.5062 72.4571 59.8737 71.5347 59.4755Z" />
                <path
                    d="M65.2653 70.2036C64.4213 70.8892 63.1883 70.7555 62.5104 69.9176C61.8247 69.0751 61.9446 67.8437 62.7886 67.158C63.6326 66.4724 64.8656 66.6061 65.5482 67.4363C66.0832 68.0881 66.1247 68.9798 65.7281 69.6624C65.6082 69.8669 65.456 70.0514 65.2653 70.2036Z" />
                <path
                    d="M74.6971 46.0435C73.6086 46.1312 72.6724 45.3256 72.5801 44.2448C72.4971 43.1702 73.2981 42.2201 74.3789 42.1278C75.4627 42.0494 76.4128 42.852 76.4958 43.9266C76.5266 44.3447 76.4282 44.7322 76.2299 45.0735C75.9178 45.6115 75.3628 45.9974 74.6971 46.0435Z" />
                <path
                    d="M51.9655 76.4885C50.9032 76.7053 49.8716 76.0258 49.6548 74.965C49.4381 73.9026 50.1222 72.8618 51.183 72.6451C52.2438 72.4283 53.2861 73.1124 53.5029 74.1732C53.5967 74.6621 53.509 75.1464 53.2753 75.5476C53.0032 76.0181 52.5436 76.3778 51.9655 76.4885Z" />
                <path
                    d="M71.247 32.6776C70.3307 33.2526 69.1192 32.982 68.5396 32.0534C67.9647 31.1372 68.2429 29.9303 69.1592 29.3538C70.0755 28.7788 71.2869 29.0479 71.8619 29.9641C72.2647 30.6083 72.2555 31.3908 71.8988 32.0058C71.7451 32.2702 71.5237 32.4962 71.247 32.6776Z" />
                <path
                    d="M37.2726 75.8936C36.2257 75.5938 35.6261 74.5038 35.9336 73.4615C36.2365 72.4284 37.3265 71.8288 38.3657 72.1224C39.4081 72.4299 40.0076 73.5199 39.7002 74.5622C39.6556 74.7191 39.5925 74.8651 39.5188 74.9927C39.0714 75.7614 38.1582 76.1565 37.2726 75.8936Z" />
                <path
                    d="M59.3016 23.1567C59.2847 23.1475 59.2678 23.1367 59.2509 23.1275C59.2417 23.1229 59.2248 23.1121 59.2079 23.1029C58.2639 22.5664 57.9457 21.3657 58.4899 20.4279C59.0264 19.4839 60.2271 19.1641 61.1711 19.7007C61.188 19.7115 61.2003 19.7284 61.2172 19.7391C61.2341 19.7483 61.2556 19.7499 61.2725 19.7606C62.2119 20.3049 62.5255 21.5148 61.9859 22.4449C61.4417 23.3827 60.241 23.7009 59.3016 23.1567Z" />
                <path
                    d="M27.3104 68.4805C26.5586 69.2661 25.3149 69.2845 24.5385 68.5374C23.7545 67.7856 23.7283 66.5495 24.4724 65.7609C25.2334 64.9814 26.4679 64.9568 27.2535 65.7086C27.9207 66.3481 28.0407 67.3413 27.5948 68.1084C27.5195 68.2376 27.4226 68.3652 27.3104 68.4805Z" />
                <path
                    d="M46.8827 19.5458C45.7973 19.5089 44.9564 18.5988 45.001 17.518C45.0379 16.4326 45.948 15.5917 47.0334 15.6271C48.1141 15.6716 48.9597 16.5741 48.9228 17.6595C48.9105 17.9946 48.8121 18.3021 48.6538 18.5757C48.3032 19.183 47.6376 19.5843 46.8827 19.5458Z" />
                <path
                    d="M19.1594 57.339C18.1247 57.6865 17.0178 57.1361 16.6796 56.1061C16.3321 55.0714 16.8841 53.9645 17.9172 53.6186C18.9457 53.2804 20.0526 53.8307 20.4 54.8654C20.583 55.4158 20.5061 56 20.234 56.4689C20.0019 56.8686 19.6329 57.1914 19.1594 57.339Z" />
                <path
                    d="M34.3087 22.1424C33.334 22.6036 32.161 22.1855 31.6921 21.2062C31.2309 20.2315 31.6491 19.0584 32.6238 18.5972C33.6046 18.1268 34.7761 18.545 35.2373 19.5212C35.5309 20.1361 35.4787 20.8141 35.1681 21.3522C34.9744 21.6843 34.6792 21.9579 34.3087 22.1424Z" />
                <path
                    d="M17.1008 43.6963C16.0277 43.5195 15.3036 42.5064 15.4758 41.441C15.6541 40.3679 16.658 39.6392 17.7311 39.816C18.7949 39.9882 19.5237 40.9936 19.3561 42.0698C19.3192 42.3111 19.2346 42.5356 19.1147 42.7401C18.7196 43.4257 17.9294 43.8239 17.1008 43.6963Z" />
                <path
                    d="M24.3849 30.2595C23.7346 31.125 22.5031 31.3018 21.6314 30.6592C20.7659 30.0089 20.5891 28.7775 21.241 27.9104C21.8836 27.0387 23.115 26.8634 23.9898 27.5199C24.7969 28.1133 25.0029 29.2156 24.5079 30.0689C24.4679 30.1365 24.4325 30.1965 24.3849 30.2595Z" />
            </g>
        </svg>

    </div>


    <div class="container">
        <div class="row d-flex justify-content-center">
            <div class="col-sm-12 col-md-10 col-lg-8 col-xl-6">
                <div class="section-title1">
                    <h2>Our Recent News</h2>
                    <p class="mb-0">Explore the world&#039;s best and largest bidding marketplace with our beautiful bidding products We want to be a part of your laughter, success and future growth.</p>
                </div>
            </div>
        </div>
        <div class="row d-flex justify-content-center g-4">
                                                <div class="col-lg-4 col-md-6 col-sm-10">
                        <div class="single-blog-style1 wow animate fadeInDown" data-wow-duration="1.5s"
                            data-wow-delay="0.2s">
                            <div class="blog-img">
                                <a href="blog/navigating-the-currents-of-electronic-ingenuity-2" class="blog-date"><i
                                        class="bi bi-calendar-check"></i>Dec 12, 2023</a>
                                <img alt="image" src="uploads/blog/1-1702826823.jpg"
                                    height="340" width="100%">
                            </div>
                            <div class="blog-content">
                                <h5><a
                                        href="blog/navigating-the-currents-of-electronic-ingenuity-2">Navigating the Currents of Electronic Ingenuity</a>
                                </h5>
                                <div class="blog-meta">
                                    <div class="author">

                                                                                    <img alt="image"
                                                src="uploads/users/eg-logo-1702294552.png">
                                                                                <a href="blog/navigating-the-currents-of-electronic-ingenuity-2"
                                            class="author-name">Bidout Apps</a>
                                    </div>
                                    <div class="comment">
                                        <img alt="image"
                                            src="frontend/images/icons/comment-icon.svg">
                                        <a href="blog/navigating-the-currents-of-electronic-ingenuity-2"
                                            class="comment">02
                                            Comments</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                                    <div class="col-lg-4 col-md-6 col-sm-10">
                        <div class="single-blog-style1 wow animate fadeInDown" data-wow-duration="1.5s"
                            data-wow-delay="0.3s">
                            <div class="blog-img">
                                <a href="blog/unveiling-the-charms-of-antique-splendors.php" class="blog-date"><i
                                        class="bi bi-calendar-check"></i>Dec 11, 2023</a>
                                <img alt="image" src="uploads/blog/4-1702297849.jpg"
                                    height="340" width="100%">
                            </div>
                            <div class="blog-content">
                                <h5><a
                                        href="blog/unveiling-the-charms-of-antique-splendors.php">Unveiling the Charms of Antique Splendors</a>
                                </h5>
                                <div class="blog-meta">
                                    <div class="author">

                                                                                    <img alt="image"
                                                src="uploads/users/eg-logo-1702294552.png">
                                                                                <a href="blog/unveiling-the-charms-of-antique-splendors.php"
                                            class="author-name">Bidout Apps</a>
                                    </div>
                                    <div class="comment">
                                        <img alt="image"
                                            src="frontend/images/icons/comment-icon.svg">
                                        <a href="blog/unveiling-the-charms-of-antique-splendors.php"
                                            class="comment">02
                                            Comments</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                                    <div class="col-lg-4 col-md-6 col-sm-10">
                        <div class="single-blog-style1 wow animate fadeInDown" data-wow-duration="1.5s"
                            data-wow-delay="0.4s">
                            <div class="blog-img">
                                <a href="blog/journey-through-the-ages-with-antique-treasures.php" class="blog-date"><i
                                        class="bi bi-calendar-check"></i>Dec 11, 2023</a>
                                <img alt="image" src="uploads/blog/7%20(1)-1702297731.jpg"
                                    height="340" width="100%">
                            </div>
                            <div class="blog-content">
                                <h5><a
                                        href="blog/journey-through-the-ages-with-antique-treasures.php">Journey Through the Ages with Antique Treasures</a>
                                </h5>
                                <div class="blog-meta">
                                    <div class="author">

                                                                                    <img alt="image"
                                                src="uploads/users/eg-logo-1702294552.png">
                                                                                <a href="blog/journey-through-the-ages-with-antique-treasures.php"
                                            class="author-name">Bidout Apps</a>
                                    </div>
                                    <div class="comment">
                                        <img alt="image"
                                            src="frontend/images/icons/comment-icon.svg">
                                        <a href="blog/journey-through-the-ages-with-antique-treasures.php"
                                            class="comment">00
                                            Comment</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                                    </div>
    </div>
</div>
                                <!-- =============== counter-section start =============== -->
<div class="about-us-counter pt-120">
    <div class="container">
        <div class="row g-4 d-flex justify-content-center">
                            <div class="col-lg-3 col-md-6 col-sm-10">
                    <div class="counter-single text-center d-flex flex-row hover-border1">
                                                    <div class="counter-icon"> <img src="uploads/fun_facts/employee-1694260644.svg"> </div>
                                                <div class="coundown d-flex flex-column">
                            <h3 class="odometer" data-odometer-final="5400"></h3>
                            <p>Happy Customer</p>
                        </div>
                    </div>
                </div>
                            <div class="col-lg-3 col-md-6 col-sm-10">
                    <div class="counter-single text-center d-flex flex-row hover-border1">
                                                    <div class="counter-icon"> <img src="uploads/fun_facts/review-1694260644.svg"> </div>
                                                <div class="coundown d-flex flex-column">
                            <h3 class="odometer" data-odometer-final="1250"></h3>
                            <p>Good Reviews</p>
                        </div>
                    </div>
                </div>
                            <div class="col-lg-3 col-md-6 col-sm-10">
                    <div class="counter-single text-center d-flex flex-row hover-border1">
                                                    <div class="counter-icon"> <img src="uploads/fun_facts/smily-1694260644.svg"> </div>
                                                <div class="coundown d-flex flex-column">
                            <h3 class="odometer" data-odometer-final="4250"></h3>
                            <p>Winner Customer</p>
                        </div>
                    </div>
                </div>
                            <div class="col-lg-3 col-md-6 col-sm-10">
                    <div class="counter-single text-center d-flex flex-row hover-border1">
                                                    <div class="counter-icon"> <img src="uploads/fun_facts/comment-1694260644.svg"> </div>
                                                <div class="coundown d-flex flex-column">
                            <h3 class="odometer" data-odometer-final="500"></h3>
                            <p>New Comments</p>
                        </div>
                    </div>
                </div>
                    </div>
    </div>
</div>

<!-- =============== counter-section end =============== -->
                    </div>

        <!-- =============== counter-section end =============== -->


<?php include 'addon/footer.php'; ?>